...
class GoodsModel extends Model {
    ...
    //后台搜索
    public function search($pageSize = 2, $isDelete = 0) {
        /*         * ************************************** 搜索 *************************************** */
        //只取出未放到回收站的数据
    ...
    protected function _after_insert($data, $option) {
        ...
        if ($mp) {
            ...
            foreach ($mp as $k => $v) {
                if (empty($v))//如果为空跳过处理下一个,trim去掉左右空格
                    continue;
                $mpModel->add(array(
                    'goods_id' => $data['id'],
                    'level_id' => $k,
    ...
    protected function _before_update(&$data, $option) {
        ...
        if (!isset($_POST['is_promote']))
            ...
            $data['promote_start_time'] = strtotime(I('post.promote_start_time')); //转化成时间戳
