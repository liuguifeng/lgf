...
//		mysql_query('START TRANSACTION');
	...
	public function setColect()
	{
		...
		if($data['token']==$token){
			...
			$scdata=$scModel->field('movie_id')->where(array('member_id'=>array('eq',$data1['member_id'])))->select();
			foreach($scdata as $k => $v){
				$arr[]=$v['movie_id'];
			}
			if(in_array($data1['movie_id'],$arr)){
				$this->error=array(4015=>"你已经收藏过了");
				return false;
			}
			else{
        ...
        public function cancle(){
            ...
            if($data['token']==$token){
                $colmdel=D("Admin/Collection");
                $where['member_id']=array('eq',$id);
                $where['movie_id']=array('in',$arr);
                $res=$colmdel->where($where)->delete();
                if($res!==false){
                    return true;
                }
                else{
                    $this->error=array(4025=>"取消收藏失败");
                }
            }
        }
}