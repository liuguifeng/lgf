...
class CommentController extends BaseController 
{
        ...
        if(IS_POST){
            ...
            if($model->create(I('post.'),1)){
                if($model->add()){
                    ...
                    //没有头像返回默认头像
                    $realFace = !$face['face'] ? '/Public/Home/images/default.jpg' : '/Public/Home/'.$face['face'];
                     echo json_encode(array(
                         'ok'=>1,
                         'content'=>I('post.content'),//过滤后的内容
    ...
    public function ajaxGetComment()
	{
		...
		foreach ($data as $k => $v)
		{
			$data[$k]['face'] = $v['face'] ? '/Public/Home/'.$v['face'] : '/Public/Home/images/default.jpg';
